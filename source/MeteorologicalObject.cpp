...
MeteorologicalObject::MeteorologicalObject()
  : itsTimeSynchronizedFeatureMembers()
{
}
...
void MeteorologicalObject::addTimeSynchronizedFeature(ParameterTimeSeriesPoint * theFeature)
{
  if (theFeature)
	...
	throw std::runtime_error("addTimeSynchronizedFeature: the feature is not a ParameterTimeSeriesPoint object");
}
...
//for (itts = tsbeg; (itts != tsend); itts++) printf("> Sync add %s\n",to_iso_string(itts->validTime()).c_str());
		...
		for (itpp = ppbeg, itti = tibeg; (itpp != ppend); itpp++) {
			...
			for (itts = pitts = tsbeg,itti = tibeg; ((itti != tiend) && (!missing)); itts++)
				if ((itts == tsend) || (itts->validTime() > *itti)) {
					...
					//
//printf("> Sync itts=%s add itti=%s\n",(itts == tsend) ? "<end>": to_iso_string(itts->validTime()).c_str(),to_iso_string(*itti).c_str());
					GeophysicalParameterValueSet *pvs = new GeophysicalParameterValueSet(**(pitts->values().begin()));
