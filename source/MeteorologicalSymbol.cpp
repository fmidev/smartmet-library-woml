...
MeteorologicalSymbol::MeteorologicalSymbol()
  : itsDefinitionReferences()
  , itsLocalizedSymbolNames()
{ }
...
void MeteorologicalSymbol::addDefinitionReference(const std::string & theScheme,const std::string & theDefinitionReference)
{
	...
	if ((cols.size() != 2) || (cols[0] == "") || (cols[1] == ""))
		throw std::runtime_error("Invalid definition reference: (" + theScheme + ") " + theDefinitionReference);
